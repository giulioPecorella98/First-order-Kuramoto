...
int main() {
    ...
    initialConditions(f, g, p.thetaPoints, p.dTheta, p.frequencyPoints, 
    ...
    OrderParameter ordR =  computeR(f, g, p.thetaPoints, p.dTheta, cosine, 
                                    sine, p.frequencyPoints, p.dFrequency); 
    fwrite(&ordR.R, sizeof(double), 1, file);

    // Start the simulation
    double updateTime = 0.0;
    std::cout << "\rComputing: [" << std::string(100, ' ') << "] 0%" 
              << std::flush;
    for (int t = 0; t < p.steps; t++) {
        ...
        finiteDifference(f, fnew, g, cosine, sine, jNext, jPrev, freq, 
                         ...
                         p.frequencyPoints, p.dFrequency, p.minimumFrequency, 
                         p.dt, p.D, p.K, p.alpha);
        std::swap(f, fnew); 
        ...
        int progress = 
            static_cast<int>((static_cast<double>(t) / p.steps) * 100);
        if (progress >= 1) {
